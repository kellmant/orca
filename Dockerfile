...
RUN \
	...
	openssl req -config openssl.cnf -key private/root.key.pem -new -x509 -days 7300 -sha256 -extensions v3_ca -out certs/root.cert.pem -subj "/C=CA/ST=Denial/L=Cloud/O=${MYORG}/OU=${MYUNIT}/CN=root.${MYDOM}" && \
	...
	openssl req -config intermediate/openssl.cnf -new -sha256 -key intermediate/private/ca.key.pem -out intermediate/csr/ca.csr.pem -subj "/C=CA/ST=Denial/L=Cloud/O=${MYORG}/OU=${MYUNIT}/CN=ca.${MYDOM}" && \
	...
	openssl req -config intermediate/local.cnf -key intermediate/private/local.key.pem -new -out intermediate/csr/local.csr.pem -days 330 -subj "/C=CA/ST=Denial/L=Cloud/O=${MYORG}/OU=${MYUNIT}/CN=ca.${LOCALAZ}" && \
